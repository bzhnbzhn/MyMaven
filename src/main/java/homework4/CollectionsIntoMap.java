...
public class CollectionsIntoMap {
    ...
    public static <K, V> Map<K, V> convertCollectionsIntoMapException(List<K> collection1, List<V> collection2) {
        HashMap<K, V> collectionsIntoMap = new HashMap<>();
        if (collection1.size() > collection2.size())
            throw new IndexOutOfBoundsException("Value is not enough for the Map");
        if (collection1.size() < collection2.size())
            throw new IndexOutOfBoundsException("Key is not enough for the Map");
